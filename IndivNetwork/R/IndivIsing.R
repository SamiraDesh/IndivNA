...
IndivIsing <- function(data, 
  ...
  #this will create a data frame with each column being a vector of length x_var and each element in a column being product of one y_var with every X_var
  #so the length si the same as the number of observations/x and y vars
  xy_inter_names <- unlist(lapply(1:length(y_index), function(k) paste0(y_names[k], "-", x_names)))
  colnames(xy_inter) <- xy_inter_names
  var_mat <- cbind(y_var, x_var, xy_inter) #variable matrix
  ...
  if(networkModel=="temporal"){
    p <- nvar+length(x_index)+nvar*length(x_index) # intercept not included - temporal includes the lag 1 value of itself as well
  }else{
    p <- nvar-1+length(x_index)+(nvar-1)*length(x_index) # intercept not included - (-1) since we only include (nodes-1) variables in our model (no self lag 1 value)
  }
  
  intercepts <- betas <- lambdas <- vector("list", nvar) #creates an empty list of length nvar
  nlambdas <- rep(0, nvar)
  ...
  for(i in 1:nvar){
    ...
    if(networkModel=="temporal"){
      model <- glmnet::glmnet(x=var_mat[which(data[,timepoint] %in% times[1:(length(times)-1)]),], 
                              #the predictors/ covariates here are values of ALL nodes, individual features and interactions at all previous time points
                              y=as.factor(var_mat[which(data[,timepoint] %in% times[2:length(times)]),i]), 
                              #corresponding outcome is the value of node of interest (i) such that x values lag by 1 
                              family = "binomial", alpha = 1)
    }else{
      model <- glmnet::glmnet(x=var_mat[,-c(i,(nvar+length(x_index)*i+1):(nvar+length(x_index)*(i+1)))], 
                              #Covariates are all nodes, interactions and features except for the node of interest
                              y=as.factor(var_mat[,i]), family = "binomial", alpha = 1) 
                              #
    }
    #extracting for each node
    intercepts[[i]] <- model$a0
    betas[[i]] <- model$beta
    lambdas[[i]] <- model$lambda #tuning parameters 
  ...
  J <- matrix(0, nrow=max(nlambdas), ncol=nvar) #zero matrix - each element is the number of connected nodes/ neighbours selected by LASSO 
                                                #at a given lambda (corresponds to a row)
  
  for(i in 1:nvar){
    J[1:ncol(betas[[i]]), i] <- colSums(as.matrix(betas[[i]]) != 0) #populate J at all rows corresponding to each column (i, node)
  }
  ...
  ####################################################################################
  ## summarize results ###############################################################
  lambda.opt <- apply(EBIC, 2, which.min) #find optimal lambda corresponding to lowest EBIC
  lambda.val <- rep(NA, nvar) 
  
  estimated_thresholds <- numeric(length(lambda.opt))  ## intercept
  #i<-1
  for (i in 1:length(lambda.opt)){
    lambda.val[i] <- lambda.mat[lambda.opt[i], i] #extract optimal lambda for the specific node from the matrix
    estimated_thresholds[i] <- intercepts[[i]][lambda.opt[i]] #intercept value corresponding to the optimal lambda
  }
  
  estimated_coeff_raw <- matrix(NA, nrow = nvar, ncol = ncol(var_mat))
  for(i in 1:nvar){
    if(networkModel=="temporal"){
      estimated_coeff_raw[i,] <- betas[[i]][,lambda.opt[i]] #extract edge weights at optimal lambda value
    }else{
      estimated_coeff_raw[i, seq(1,ncol(var_mat),1)[-c(i,(nvar+length(x_index)*i+1):(nvar+length(x_index)*(i+1)))]] <- betas[[i]][,lambda.opt[i]]  
    }                                             #remove outcome associated covariates in the static model
  }
