...
IndivRep <- function(data,
  ...
  for(i in 1:rep){
    ...
    if(is.na(timepoint)){
      ...
      train_index <- sample(unique(data[,id]), ceiling(length(unique(data[,id]))/2), replace = FALSE)
      
      data_train <- data[data[,id] %in% train_index,]
      data_inference <- data[!(data[,id] %in% train_index),]
    }
