##UPDATED TO ACCOUNT FOR ERRORS WITH CLOSENESS SUMMARY
...
#' @export
IndivBootSummary <- function(data, #data <- data_inference
                             IsingBootResult, #IsingBootResult <- boot_result
                             IsingResult, #IsingResult <- result
                             nCores = 1,
                             library = .libPaths(),
                             verbose = TRUE,
                             sample_covar_mat){
  ...
  if(IsingBootResult$type=="nonparametric"){
    ...
    if(nCores==1){
      ...
      sfExport()
      sfLibrary()
      sfClusterApplyLB(1:(nBoots*nrow(sample_covar_mat)), function(b){
    ...
    #############CODE ADDED FROM NON-PARAMETRIC BOOTSTRAP#####################
    ##########NEED TO CHANGE SINGLE TO NESTED LAPPLY TO ACCOMODATE FOR CASE DROPPING PROBABILITY################################################################
    ## summarize bootnet quantiles ###########################################

    #k is the index of bootstrap; j is the index of casedropping probability

    thresholds <- do.call(rbind, lapply(1:nBoots, function(k)
      do.call(rbind, lapply(1:length(sample_prob), function(j) IsingBootResult$bootResults[[k]][[j]]$estimated_thresholds))))

    #each outcome has a threshold (so 10 estimates (cols) in each bootstrap sample and casedropping prob combination (5x5=25 rows))
    boot_estimates <- do.call(rbind, lapply(1:nBoots, function(k)
      do.call(rbind, lapply(1:length(sample_prob), function(j) IsingBootResult$bootResults[[k]][[j]]$estimated_coeff_raw))))
    ...
    for(i in 1:length(y_index)){
      ...
      #BIAS ASSOCIATED WITH THE OUTCOME ITSELF = THRESHOLD (NBOOT*LENGTH(SAMPLE_PROB) VALUES)
      edge_names <- c(edge_names, rep(paste0(y_names[i], "-bias"), times=(nBoots*length(sample_prob))))
      edge_weights <- c(edge_weights, thresholds[,i])

      #APPENDING ALSO THE BIAS ASSOCIATED WITH FEATURES (NBOOT*LENGTH(SAMPLE_PROB) VALUES FOR EACH Y*XN)
      edge_names <- c(edge_names, rep(paste0(y_names[i], "-bias", ":", x_names), times=(nBoots*length(sample_prob))))
      #k<- 3
      ...
      #extracting nBoots feature related bias values corresponding to the kth bootstrap for each ith outcome
      if(is.na(IsingResult$timepoint)){ #static
        for(j in i:length(y_index)){ #j<-2
          if(j > i){
            #for each of j and i, indices for itself and interactions with features
            var_index1 <- c(j, (length(y_index)+j*length(x_index)+1):(length(y_index)+(j+1)*length(x_index)))
            var_index2 <- c(i, (length(y_index)+i*length(x_index)+1):(length(y_index)+(i+1)*length(x_index)))
            ...
            if(IsingResult$AND==FALSE){
              ...
              #get edge weight estimates (ji an d ij) and take average
            }
            else if(IsingResult$AND==TRUE){
              edge_weights <- c(edge_weights, do.call(c,lapply(1:nBoots, function(k) do.call(c,lapply(1:length(sample_prob), function (q) ((boot_estimates[i+((k-1)*5+(q-1))*length(y_index),var_index] + boot_estimates[j+((k-1)*5+(q-1))*length(y_index),var_index])/2)*(boot_estimates[i+((k-1)*5+(q-1))*length(y_index),var_index]!=0 & boot_estimates[j+((k-1)*5+(q-1))*length(y_index),var_index]!=0))))))
              #take average only if both non zero
            }
          }
        }
      }
      else{ ## temporal network - don't have to worry about symmetry - no need to test for AND
    ...
    boot_data <- data.frame("edge_names" = edge_names, "edge_weights" = edge_weights)
    #Estimates for all components of the linear predictor - bias, bias-feature interaction, main effect, main-effect feature interaction
    #nBoot estimates for each
    boot_data_summary <- boot_data %>% dplyr::group_by(edge_names) %>%
      dplyr::summarise(mean=mean(edge_weights),
                       lb=quantile(edge_weights, probs=0.025, type=6),
                       ub=quantile(edge_weights, probs=0.975, type=6))
    boot_data_summary <- boot_data_summary[order(boot_data_summary$mean),]
    boot_data_summary$edge_names <- factor(boot_data_summary$edge_names, levels = boot_data_summary$edge_names)
    #BOOT_DATA_SUMMARY already has unique edge names
    gc()
    ##########################################################################
    ##########################################################################
    #SELECT EDGES ARE THOSE WHERE EDGES ARE NON ZERO
    select_edges <- NULL
    if(is.na(IsingResult$timepoint)){ #static
      for(i in 1:(length(y_index)-1)){ #i in 1:9
        for(j in (i+1):length(y_index)){ # j in 2:11
          #IsingResult$estimated_formula[i,j] <- "0.642+1.180>>X3"
          #j<-3; i<-1
          if(!is.na(IsingResult$estimated_formula[i,j])){ #if the two nodes are connected
            str_vec <- strsplit(IsingResult$estimated_formula[i,j], "+", fixed = TRUE)[[1]]
    ...
    edge_data <- boot_data[which(boot_data$edge_names %in% select_edges),]
    #All estimates (edge_weights) for the pair that is a selected edge
    edge_data$edge_names <- factor(edge_data$edge_names, levels = unique(edge_data$edge_names))
    edge_data_summary <- boot_data_summary[which(boot_data_summary$edge_names %in% select_edges),]
    #Extract and assign the summary stats for the bootstrap estimates for this pair
    edge_data_summary$edge_names <- factor(edge_data_summary$edge_names, levels = unique(edge_data_summary$edge_names))
    ...
    #Find the differences between each pair of selected edge weights -
    #This is to see if there is a significant difference between the two
    for(i in 1:(length(select_edges)-1)){ #i<-1
      for(j in (i+1):length(select_edges)){ #j<-2
        ...
        if(edge_diff_lb*edge_diff_ub>0){ #if 0 is not included in the interval - both <0 or >0
          edge_test[i,j] <- edge_test[j,i] <- 1
        }else{
          edge_test[i,j] <- edge_test[j,i] <- 0
        }
    ...
    ## use multiple core to save time
    if(nCores==1){
      casedropping_network_func <- function(kk){ #the index of bootstrap
        boot_network_func <- function(jj){ #index of casedropping prob
      ...
      boot_centrality <- do.call(rbind, lapply(1:(nBoots*length(sample_prob)*nrow(sample_covar_mat)), function(i) boot_network_list[[i]]$node_centrality))
    }
    ...
    #EXCLUDE CLOSENESS FROM CORRELATION CALCULATION
    if(IsingResult$networkModel=="static"){
      n_centrality <- c(1,3:4) #CHANGED number of centralities 4 --> 3 (1,3:4)
    }else{
      n_centrality <- c(1,3:6) #CHANGED number of centralities 6 --> 5 (1,3:6)
    }
    ...
    centrality_summary <- lapply(1:nBoots, function(k)
      sapply(1:length(sample_prob), function(j)
        sapply(1:nrow(sample_covar_mat), function(l)
          sapply(n_centrality, function(i) cor.test(boot_centrality[boot_centrality$boot==k & boot_centrality$sample_prob==sample_prob[j] & boot_centrality$id==l,i],
                                                    ind_network_list[[l]]$node_centrality[,i], method="spearman")$estimate))))
               #CHANGED index: 1:n_centrality --> n_centrality
    cor_func <- function(i){
      #CHANGED nrow: n_centrality --> length(n_centrality)
      mean_vec <- as.vector(sapply(1:length(sample_prob), function(j) apply(matrix(centrality_summary[[i]][,j], nrow=length(n_centrality), byrow = FALSE), 1, mean)))
      sd_vec <- as.vector(sapply(1:length(sample_prob), function(j) apply(matrix(centrality_summary[[i]][,j], nrow=length(n_centrality), byrow = FALSE), 1, sd)))

      if(IsingResult$networkModel=="static"){ #CHANGED list of centralities: closeness excluded
        cor_mat <- data.frame("centrality"=c("Betweenness","Strength","ExpectedInfluence"),
                              "mean"=mean_vec,
                              "sd"=sd_vec)
      }else{                                 #Closeness excluded
        cor_mat <- data.frame("centrality"=c("Betweenness","InStrength","OutStrength","OutExpectedInfluence","InExpectedInfluence"),
                              "mean"=mean_vec,
                              "sd"=sd_vec)
      }
    ...
    cor_final_results$sample_prob <- rep(sample_prob, each=length(n_centrality))  #CHANGED nrow: n_centrality --> length(n_centrality)
...
#boot_network_list[[5]][[2]] 5th bootstrap, 2nd feature vector
